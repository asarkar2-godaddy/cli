...
describe("Extension Workspace", () => {
	...
	describe("getExtensionsEffect", () => {
		test("fails when extensions directory does not exist and no workspaces", async () => {
			await expect(
				runEffect(getExtensionsEffect({ repoRoot: tempDir })),
			).rejects.toThrow("no package.json found");
		});

		test("fails when extensions path is a file, not a directory", async () => {
		...
		test("finds single extension with valid package.json", async () => {
			...
			const packageJson = { name: "@test/ext1", version: "1.0.0" };
			fs.writeFileSync(
		...
		test("fails when package.json is missing name field", async () => {
			const extensionsPath = path.join(tempDir, "extensions");
			const ext1Path = path.join(extensionsPath, "ext1");
			fs.mkdirSync(ext1Path, { recursive: true });
			...
			await expect(
				runEffect(getExtensionsEffect({ repoRoot: tempDir })),
			).rejects.toThrow('missing "name" field');
		});
	...
	describe("getExtensionsEffect - workspaces fallback", () => {
		...
		test('uses godaddy.type = "extension" to mark workspace as extension', async () => {
			const rootPackageJson = { workspaces: ["packages/*"] };
			fs.writeFileSync(
				path.join(tempDir, "package.json"),
				JSON.stringify(rootPackageJson),
			);
		...
		test("handles direct workspace paths (non-glob)", async () => {
			const rootPackageJson = { workspaces: ["packages/ext1"] };
			fs.writeFileSync(
				path.join(tempDir, "package.json"),
				JSON.stringify(rootPackageJson),
			);
		...
		test("fails when no extensions directory and no package.json", async () => {
			await expect(
				runEffect(getExtensionsEffect({ repoRoot: tempDir })),
			).rejects.toThrow("no package.json found");
		});

		test("fails when no extensions directory and no workspaces", async () => {
			fs.writeFileSync(
				path.join(tempDir, "package.json"),
			...
			await expect(
				runEffect(getExtensionsEffect({ repoRoot: tempDir })),
			).rejects.toThrow("no workspaces defined");
		});
